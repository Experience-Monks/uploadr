...
module.exports = function auth (opts, done) {
  ...
  readEnvFile(function (err, data, filepath) {
    ...
    if (opts.prompt === false) {
      // Allow no env file if there's enough info passed to authenticate
      if (opts.username && (opts.password || opts.key)) {
        if (opts.key) {
          fs.readFile(opts.key, function (keyErr, keyBuffer) {
            if (keyErr) return done(keyErr);
            data.privateKey = keyBuffer.toString();
  ...
  function runAuthSeries (usePrompt, err, data, filepath, next) {
    ...
    if (usePrompt) {
      var logText;
      if (save) logText = fileExisted ? 'Configuring .env file...' : 'Writing .env file...';
      else logText = 'Connecting...';
      log.info(logText);
      var questions = [
        {
          type: 'input',
